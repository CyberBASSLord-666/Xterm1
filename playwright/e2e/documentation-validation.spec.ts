...
function resolveLinkPath(sourceFile: string, linkUrl: string): string {
  ...
  // Remove leading ./
  const cleanUrl = decodedUrl.startsWith('./') ? decodedUrl.substring(2) : decodedUrl;
...
function findMarkdownFiles(
  dir: string,
  excludeDirs: string[] = ['node_modules', '.git', 'dist', 'coverage', 'playwright-report', 'test-results']
): string[] {
  const files: string[] = [];
...
test.describe('Documentation Validation', () => {
  ...
  test('should have no broken internal markdown links', () => {
    ...
    expect(result.brokenLinks, `Found ${result.brokenLinks.length} broken link(s) in documentation`).toHaveLength(0);
  });
  ...
  test('should have proper documentation hierarchy', () => {
    ...
    for (const [tier, files] of Object.entries(tiers)) {
      ...
      for (const file of files) {
        ...
        expect(existsSync(fullPath), `${tier} should contain ${file}`).toBeTruthy();
      }
    }
  });
  ...
  test('should have all consolidated documents with proper headers', () => {
    const consolidatedDocs = [
      ...
      },
      {
        ...
        shouldContain: ['Production Readiness', 'SECURITY.md', 'TESTING.md'],
      },
      {
        file: 'docs/reference/dependency-management.md',
        shouldContain: ['Dependency Management', 'Dependabot', '.github/dependabot.yml'],
      },
    ];
