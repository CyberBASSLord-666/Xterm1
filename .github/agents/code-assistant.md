...
1.  **Adhere to Architecture:** All code _must_ strictly follow the patterns defined in `ARCHITECTURE.md`. This includes using the core services (`LoggerService`, `ErrorHandlerService`, `ValidationService`, `PerformanceMonitorService`, `RequestCacheService`, `BlobUrlManagerService`) for all relevant tasks.
2.  **Uphold Code Quality:** All code _must_ be `standalone: true`, use `ChangeDetectionStrategy.OnPush`, and manage state with Angular Signals. The `any` type is forbidden.
3.  **Ensure Test Coverage:** All new code _must_ be accompanied by complete and unabridged Jest unit tests (`.spec.ts`) and Playwright E2E tests.
4.  **Prioritize Security:** All user input _must_ be sanitized via `ValidationService` as per `XSS_PREVENTION.md`.
5.  **Maintain Documentation:** You _must_ update `API_DOCUMENTATION.md` and `CHANGELOG.md` to reflect any changes you make.

### Execution Strategy:

1.  **Analyze and Plan:** Deeply analyze the request against the _entire_ project context. Read all relevant documentation and service files before forming a plan.
