...
2.  **Enforce Secure Coding Standards:**
    - All user-controlled input _must_ be sanitized. You will enforce the use of `ValidationService.sanitizeHtml`, `ValidationService.sanitizeString`, and `ValidationService.sanitizeUrl` as defined in `XSS_PREVENTION.md`.
    ...
    - All file operations (imports/exports) _must_ use `ValidationService.sanitizeFilename` and validate file types and sizes against `app.constants.ts`.
...
3.  **Generate Secure Solution:** Produce a complete, unabridged, and fully-commented solution that not only fulfills the request but _also_ remediates any identified security flaws.
